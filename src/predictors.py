...
#def truncated_precision(y_true, y_predict):

# class ConfusionMatrix(Callback):
#     def on_epoch_end(self, epoch, logs={}):
#         # we should print a confusion matrix...
#         mask = np.sum(self.validation_data[1], axis=2)
#         y_true_labels = mask * (np.argmax(self.validation_data[1], axis=2) + 1)
#         raw_predictions = self.model.predict(self.validation_data[0])
#         y_pred_labels = mask * (np.argmax(raw_predictions, axis=2) + 1)
...
#                 if int(y_true_labels[i, j]) == 0:
#                     break
#                 #print '{} {}'.format(int(y_true_labels[i, j]), int(y_pred_labels[i, j]))
#                 confusion[int(y_true_labels[i, j]) - 1][int(y_pred_labels[i, j]) - 1] += 1
#         print confusion
...
# class EncoderDecoder(object):
#     def __init__(self, max_seq_length, batch_size=100):
#         self.model = Sequential()
#         self.max_seq_length = max_seq_length
#         self.batch_size = batch_size
#         self.add_encoder()
...
#     def train(self, x_train, y_train, train_lengths, unclassified_weight=1.0, num_epochs=20, batch_size=50):
...
#         # This step is valid for weighting the classes
#         # while computing cross-entropy loss
#         unclassified_idx = LABEL_SET.index(UNCLASSIFIED)
#         mod_y_train = y_train[:, :, :]
#         mod_y_train[:, :, unclassified_idx] *= unclassified_weight

#         self.model.fit(x_train, mod_y_train,
#           batch_size=self.batch_size,
#           epochs=num_epochs,
